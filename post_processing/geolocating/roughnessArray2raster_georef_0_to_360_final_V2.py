...
roughness_dir_name = 'roughArrays_40_173_174_175_fullSet'  # subdir that includes roughness imgs

# tiff dir; where arr2tiff goes to, for now se build it inside rouhness dir
georefRaster_dir_name = 'roughArrays_40_173_174_175_fullSet_reproPolar_float64_fewGCPs_test3'
########################################################################################################################
#~ global IDfiers
########################################################################################################################
# path_num = 40
...
def main():
	...
	for file_count, rough_fname in enumerate(rough_files_fullpath_list):
		...
		if (skip_meridian=='on'):
			...
			# make 2 copies of blocks that cross A.M., then mask each for + and - long. and write that block 2 times with masked + and - sides 
			# note: all raster files will be in range: [0, +180] and [0, -180], so change the rest of the code where lon. are get changed

		if (antimaridina_crossing==True):
			translated_img_1_fullpath, translated_img_2_fullpath = apply_gcp_AntiMeridianCrossing()  # inside here we make a copy of AN crossing raster and mask each siode of AM line, one for + lon. and one for - lon. this f() should return 2 translated images
			...
			translated_img_fullpath = apply_gcp_normal(path_label, block_label, image_dir, in_ds, gcp_list)
...
#             print("\n-> processing img coords: (%d, %d)" %(jrow, icol))
	...
	if (any( [pixel_LatLon_tuple[1] < 0  for  pixel_LatLon_tuple in img_worldFrame_latlon_list] )):   # if any Lon in img_worldFrame_latlon_list is neg. we change that to pos.
		...
		if (all([pixel_LatLon_tuple[1] < 0  for  pixel_LatLon_tuple in img_worldFrame_latlon_list])):
			...
			print('-> West Hem. all negative lon! we change all to range: [0-360]') # update: not change anymore
...
def apply_gcp_AntiMeridianCrossing(path_label, block_label, image_dir, in_ds, gcp_list):
	...
	copied_translated_img_fullpath = out_translated_img_fullpath
...
def apply_gcp_normal(path_label, block_label, image_dir, in_ds, gcp_list):
	#~ gdal.translate to convert data set formats (how input data from memory???)
	#~ write/save image array into raster ??????
