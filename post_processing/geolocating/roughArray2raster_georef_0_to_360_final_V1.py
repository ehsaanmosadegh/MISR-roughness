...
roughness_dir_name = 'roughArrays_40_173_174_175_smaller_set'  # subdir that includes roughness imgs

# tiff dir; where arr2tiff goes to, for now se build it inside rouhness dir
georefRaster_dir_name = 'roughArrays_40_173_174_175_smaller_set_reproPolar_test1'
########################################################################################################################
#~ global IDfiers
########################################################################################################################
# path_num = 40
...
def main():
	...
	for file_count, rough_fname in enumerate(rough_files_fullpath_list):
		...
		gcp_list, total_gcps, maridina_crossing = create_gcp_list_for_imgBlockPixels(path_num, block_num, misr_res_meter)

		if (skip_meridian=='on'):
			if (maridina_crossing==True):
		...
		warpedFile_fullPath_noExt = warp_img(path_label, block_label, total_gcps, image_dir, translated_img_fullpath)
...
def arr2img_plot_n_save(in_arr_2d, path_label, block_label, img_dir):
	...
	if (in_arr_2d.max() < 1):
		...
		plt.imsave(out_img_fullpath, in_arr_2d)
...
def read_rough_file(rough_fname):
	rough_1d_arr = np.fromfile(rough_fname, dtype=np.double) # 'float64'     # is this roughness in cm?
...
def create_gcp_list_for_imgBlockPixels(path_num, block_num, misr_res_meter):
	#~ create list of GCPs 
...
#             print("\n-> processing img coords: (%d, %d)" %(jrow, icol))
	...
	return gcp_list, len(img_pixelFrame_rowcol_list), maridina_crossing
########################################################################################################################
...
def warp_img(path_label, block_label, total_gcps, image_dir, translated_img_fullpath):
	'''
	...
	'''
	warped_img_tag = 'projected_'+path_label+'_'+block_label+'_'+str(total_gcps)+"GCPs_noGdalRes"
	...
	cmd = 'gdalwarp\
		...
		-srcnodata 0 \
		-dstnodata 0 ' + translated_img_fullpath + ' ' + out_warped  
	#~ notes: 
	...
	print('-> output warped file: %s' % out_warped)
