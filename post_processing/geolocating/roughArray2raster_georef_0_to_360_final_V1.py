...
roughness_dir_name = "multithreaded_atmmodel_newASCM_testPaths173_4_5"  # subdir that includes roughness imgs

# tiff dir; where arr2tiff goes to, for now se build it inside rouhness dir
georefTif_dir_name = 'outDType_Byte_range_0_360'
########################################################################################################################
#~ global IDfiers
########################################################################################################################
# path_num = 40
...
def arr2img_plot_n_save(in_arr_2d, path_label, block_label, img_dir):
    ...
    if (in_arr_2d.max() < 1):
        ...
        if (save_mode==True):
            ...
            out_img = path_label+'_'+block_label+img_format
...
#             print("\n-> processing img coords: (%d, %d)" %(jrow, icol))
    ...
    if (any( [pixel_LatLon_tuple[1] < 0  for  pixel_LatLon_tuple in img_worldFrame_latlon_list] )):   # if any Lon in img_worldFrame_latlon_list is neg. we change that to pos.
        ...
        if (all([pixel_LatLon_tuple[1] < 0  for  pixel_LatLon_tuple in img_worldFrame_latlon_list])):
            ...
            print('-> West Hem. all!')
            #~ all lon. on W.H. side, change to range [0, 360]
            ...
            print('-> image is A.M. crossing!')
            #~ not all lon. on W.H. side, change to range [0, 360]
            gcp_list = []  # a list of ground control points
            ...
            print('-> image is P.M. crossing!')
            #~ for PM crossing we do not do any conversion
            gcp_list = []  # a list of ground control points
    ...
    return gcp_list, len(img_pixelFrame_rowcol_list)
########################################################################################################################
