...
georefTif_dir_name = 'test_outDType_reprojectionToPolar_0_180'
########################################################################################################################
#~ global IDfiers
########################################################################################################################
path_num = 40
...
def main():
    ...
    for file_count, rough_fname in enumerate(rough_files_fullpath_list):
        ...
        ret = arr2img_plot_n_save(rough_arr_2d, block_label, image_dir)
        ...
        translated_img_fullpath = apply_gcp(block_label, image_dir, in_ds, gcp_list)

        warped_file_noExt = warp_img(block_label, total_gcps, image_dir, translated_img_fullpath)

        if (reprojection == 'on'):
            target_epsg = 3995
            print('-> reprojecting to: %s' % target_epsg)
            file_extension = '.tif'
            input_raster = warped_file_noExt+file_extension
            output_raster = warped_file_noExt+'_reprojectedto_'+'EPSG_'+str(target_epsg)+file_extension
...
def img_dir_setup(georefTif_dir_path, georefTif_dir_name):
    img_dir = os.path.join(georefTif_dir_path, georefTif_dir_name) 
...
def warp_img(block_label, total_gcps, image_dir, translated_img_fullpath):
    '''
    ...
    '''
    warped_img_tag = 'projected_'+block_label+'_'+str(total_gcps)+"GCPs_noGdalRes"
    warped_img_extension = ".tif"
    warped_img = warped_img_tag+warped_img_extension
    out_warped = os.path.join(image_dir, warped_img)

    warped_file_noExt = os.path.join(image_dir, warped_img_tag)
