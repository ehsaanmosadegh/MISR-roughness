...
double *brfcf1 = 0, *brfcf2 = 0, *tmp = 0; // *sz = 0; *sa = 0; E:turnedoff
//double radiance, solarAzimuth, solarZenith, terrainAspect, terrainSlope, p180 = M_PI / 180.0, cosi;
double radiance, p180 = M_PI / 180.0, cosi; // solarZenith, solarAzimuth E: turnedoff
...
if (databuf.nline != PNG_NLINES || databuf.nsample != PNG_NSAMPLES) // left here only for error checking
	{
...
if (n > 0)
	...
	//sa = zoomArray(tmp, 8, 32, 64); E: turnedoff
	// sa = zoomArray(tmp, 8, 32, ZOOM); E: turnedoff
	// if (!sa) return 0; E: turnedoff
	// if (wrap) E: turnedoff
	// 	{
...
for (j = 0; j < nlines; j ++)
	for (i = 0; i < nsamples; i ++)
		...
		// 	continue;
		// 	}
		// solarZenith = sz[i + j * 2048]; AN
		// solarZenith = sz[i + j * PNG_NSAMPLES]; E: turnedoff
		// if (solarZenith == NO_DATA)
		// 	{
		// 	data[i + j * nsamples] = NO_DATA; E: turnedoff
		// 	continue;
		// 	}
		// if (!getAspectSlope(j, i, &terrainAspect, &terrainSlope)) return 0; E: turnedoff
...
for (j = 0; j < nlines; j ++) // Q- QA?
	for (i = 0; i < nsamples; i ++)
		{ // QA check?
		if (data[i + j * nsamples] == NO_DATA) continue; // what is this slice?
		// if (data[i + j * nsamples] == TDROPOUT) continue; E: turnedoff
...
if (nvalid > 0) 
	...
	if (nvalid > 1)
		...
		for (i = 0; i < nlines * nsamples; i ++)
			...
			if (z == BACKGROUND) continue;
			if (z == FOREGROUND) continue;
			//if (z == TDROPOUT) continue; E: turnedoff
	...
	for (i = 0; i < nsamples; i ++)
		...
		for (j2 = -w2; j2 <= w2; j2 ++)
			for (i2 = -w2; i2 <= w2; i2 ++)
				...
				i3 = i + i2;
				j3 = j + j2;
				// if (j3 >= 0 && j3 < nlines && i3 >= 0 && i3 < nsamples && data[i3 + j3 * nsamples] == TDROPOUT) // E: turnedoff
        if (j3 >= 0 && j3 < nlines && i3 >= 0 && i3 < nsamples && data[i3 + j3 * nsamples] == NO_DATA) // E: updated to NO_DATA

					n ++;
				}
		// if (n > 0) data2[i + j * nsamples] = TDROPOUT; E: turnedoff
...
//if (argc < 4)
if (argc < 6) // this might happen later, cos I turnedoff fname[2]
	{
...
// strcpy(fname[2], argv[6]);		// str- image file; E: turnedoff 
...
if (nvalid > 0)
	...
	printf("%03d  %06d  %03d  %s  %5d  %5d  %10d  %14.6f  %14.6f  %14.6f  %14.6f  %14.6f \n", 
		path, orbit, block, camera == 1 ? "cf" : camera == 4 ? "an" : camera == 7 ? "ca" : camera == 2 ? "bf" : camera == 3 ? "af" : camera == 5 ? "aa" : camera == 6 ? "ba" : "??", 
		nlines, nsamples, nvalid, min, max, mean, stddev, meanSZ); // E: removed ndropouts, %10d from output
