...
if (VERBOSE) fprintf(stderr, "readTerrainFile: fname=%s, block=%d, band=%d\n", fname, block, band);

status = MtkFileType(fname, &filetype);
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkFileType failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
...
if (filetype != MTK_GRP_ELLIPSOID_GM)
	{
	fprintf(stderr, "readTerrainFile: only L1B2 global mode TERRAIN files are supported!!!\n");
	return 0;
	}

strcpy(gridName, "GeometricParameters"); 
strcpy(fieldName, "SolarZenith");
if (VERBOSE) fprintf(stderr, "readTerrainFile: grid=%s, field=%s\n", gridName, fieldName);
status = MtkReadBlock(fname, gridName, fieldName, block, &szbuf);
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkReadBlock1 failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
if (VERBOSE) fprintf(stderr, "readTerrainFile: nline=%d, nsample=%d, datasize=%d, datatype=%d (%s)\n", 
	szbuf.nline, szbuf.nsample, szbuf.datasize, szbuf.datatype, types[szbuf.datatype]);
if (szbuf.nline != 8 || szbuf.nsample != 32)
	{
	fprintf(stderr, "readTerrainFile: %s is not 8x32: (%d, %d)\n", fieldName, szbuf.nline, szbuf.nsample);
	return 0;
	}
tmp = (double *) malloc(8 * 32 * sizeof(double));
if (!tmp)
	{
	fprintf(stderr, "readTerrainFile: tmp malloc failed!!!\n");
	return 0;
	}
n = 0;
for (j = 0; j < 8; j ++)
...
if (n != 256)
	if (VERBOSE) fprintf(stderr, "readTerrainFile: fewer than 256 valid in %s: %d\n", fieldName, n);
if (n > 0)
	{
...
if (band == 0) { strcpy(gridName, "BlueBand"); strcpy(fieldName, "Blue Radiance/RDQI"); }
	...
	{
	fprintf(stderr, "readTerrainFile: unsupported band = %d\n", band);
	return 0;
	}
...
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkReadBlock2 failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
...
if (databuf.nline != PNG_NLINES || databuf.nsample != PNG_NSAMPLES)
	{
	fprintf(stderr, "readTerrainFile: databuf is not 512x2048: (%d, %d)\n", databuf.nline, databuf.nsample);
	return 0;
	}
...
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkFillValueGet failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
...
if (VERBOSE) fprintf(stderr, "readTerrainFile: FillValue: nline=%d, nsample=%d, datasize=%d, datatype=%d (%s), value=%u\n", 
...
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkGridAttrGet failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
if (VERBOSE) fprintf(stderr, "readTerrainFile: Scale factor: nline=%d, nsample=%d, datasize=%d, datatype=%d (%s), value=%f\n", 
...
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkReadBlock3 failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
...
if (cfbuf.nline != 8 || cfbuf.nsample != 32)
	{
	fprintf(stderr, "readTerrainFile: cfbuf is not 8x32: (%d, %d)\n", cfbuf.nline, cfbuf.nsample);
	return 0;
	}
brfcf1 = (double *) malloc(8 * 32 * sizeof(double));
if (!brfcf1)
	{
	fprintf(stderr, "readTerrainFile: brfcf malloc failed!!!\n");
	return 0;
	}
n = 0;
for (j = 0; j < 8; j ++)
...
if (n != 256) 
	if (VERBOSE) fprintf(stderr, "readTerrainFile: fewer than 256 valid in cfbuf: %d\n", n);
if (n > 0)
	{
...
if (status != MTK_SUCCESS) 
	{
	fprintf(stderr, "readTerrainFile: MtkReadBlock4 failed!!!, status = %d (%s)\n", status, errs[status]);
	return 0;
	}
if (VERBOSE) fprintf(stderr, "readTerrainFile: nline=%d, nsample=%d, datasize=%d, datatype=%d (%s)\n", 
...
if (!data)
	{
	fprintf(stderr, "readTerrainFile: data malloc failed!!!\n");
	return 0;
	}
...
if (minnaert) {
	...
	for (i = 0; i < nsamples; i ++)
		...
		if (cosi <= 0.0) 
			{
			//fprintf(stderr, "readTerrainFile: cosi = %14.6f, ta = %14.6f, sa = %14.6f, ta - sa = %14.6f\n", 
...
if (VERBOSE) fprintf(stderr, "readTerrainFile: data is %d x %d\n", nlines, nsamples);
...
block = atoi(argv[2]);
band = atoi(argv[3]);
minnaert = atoi(argv[1]);
strcpy(fname[1], argv[5]);
strcpy(fname[2], argv[6]);
