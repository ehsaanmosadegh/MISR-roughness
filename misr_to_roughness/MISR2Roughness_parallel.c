...
<<<<<<< HEAD
#define total_threads 20         // q- how many threads for my mac? how many logical processors?  how many for HPC? number of cpu cores on HPC?
=======
...
>>>>>>> 4376033f34f57a8c988d83db2373d2fd65b36bea
...
int main() {
   ...
   // char atmmodel_csvfile[256] = "/home/mare/Projects/MISR/Julienne/IceBridge2016/SeaIce_Jul2016_atmmodel_csvfile2_r025.csv"; // ATM csv file; source from where/
	...
	if (dirp) {     // if ptr available == TRUE
		while ((DirEntryObj = readdir(dirp)) != NULL) {     // read the first item in dir and moves the ptr to next item/file in dir; DirEntryObj == struct==fileObj for each file in dir
			...
			if (toa_an_files_list_ptr == 0) {  // if it has not been created yet
				...
				toa_an_files_list_ptr = (char **) realloc(toa_an_files_list_ptr, (tot_toa_an_files + 1) * sizeof(char *));
			...
			toa_an_files_list_ptr[tot_toa_an_files] = (char *) malloc(strlen(DirEntryObj->d_name) + 1); // allocate mem-

			if (!toa_an_files_list_ptr[tot_toa_an_files]) {
			...
			strcpy(toa_an_files_list_ptr[tot_toa_an_files], DirEntryObj->d_name);  // fill the toa_an_files_list_ptr with d_name: misr surf files; from DirEntryObj=fileObj gets the fileName
			// printf("FOUND: d_name == TOA file: %s \n", DirEntryObj->d_name);       // d_name is char array inside <dirent.h>
			//printf("file no. %d, %s \n", tot_toa_an_files, toa_an_files_list_ptr[tot_toa_an_files]);
			tot_toa_an_files ++; // counter of num of MISR files found == elements in toa_an_files_list_ptr
		}
		closedir (dirp);
	} 
	else {
	...
	but we use batches of toa files now. w/ batches we'll be able to run in a multithreading cycle */
	for (int batch_iter = 0; batch_iter < total_batches; batch_iter++) { 
		...
		for (int thread_iter = 0 ; thread_iter < total_threads; thread_iter++) { 
			...
			// todo: access() to check if input file exists
			...
			toaFile_struct_ptr[thread_iter]->total_toa_files = tot_toa_an_files;
